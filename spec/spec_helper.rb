...
RSpec.configure do |config|
  ...
  # Remove sentinel lockfiles before the suite starts to avoid stale state
  config.before(:suite) do
    begin
      root = File.expand_path("..", __dir__)
      Dir.glob(File.join(root, ".floss_funding.*.lock")).each { |f| FileUtils.rm_f(f) }
    rescue StandardError => e
      warn "pre-run sentinel cleanup warning: #{e.class}: #{e.message}"
    end
  end
  ...
  # Cleanup autogenerated gems after the entire suite and remove sentinels again
  config.after(:suite) do
    begin
      # Remove sentinel lockfiles again at the end of the run
      root = File.expand_path("..", __dir__)
      Dir.glob(File.join(root, ".floss_funding.*.lock")).each { |f| FileUtils.rm_f(f) }
